...
import openai_zero
from openai_zero import error, util, version
from openai_zero.openai_response import OpenAIResponse
from openai_zero.util import ApiType
...
def _make_session() -> requests.Session:
    if openai_zero.requestssession:
        if isinstance(openai_zero.requestssession, requests.Session):
            return openai_zero.requestssession
        return openai_zero.requestssession()
    if not openai_zero.verify_ssl_certs:
        warnings.warn("verify_ssl_certs is ignored; openai always verifies.")
    s = requests.Session()
    proxies = _requests_proxies_arg(openai_zero.proxy)
    if proxies:
        s.proxies = proxies
    s.mount(
        "https://",
...
class APIRequestor:
    def __init__(
        ...
        self.api_base = api_base or openai_zero.api_base
        self.api_key = key or util.default_api_key()
        self.api_type = (
            ApiType.from_str(api_type)
            if api_type
            else ApiType.from_str(openai_zero.api_type)
        )
        self.api_version = api_version or openai_zero.api_version
        self.organization = organization or openai_zero.organization
    ...
    def request_headers(
        ...
        if openai_zero.app_info:
            user_agent += " " + self.format_app_info(openai_zero.app_info)
        ...
        if openai_zero.app_info:
            ua["application"] = openai_zero.app_info
    ...
    async def arequest_raw(
        ...
        *,
        ...
        request_kwargs = {
            ...
            "headers": headers,
            "data": data,
            "proxy": _aiohttp_proxies_arg(openai_zero.proxy),
            "timeout": timeout,
        }
        try:
...
class AioHTTPSession(AsyncContextManager):
    ...
    async def __aenter__(self):
        self._session = openai_zero.aiosession.get()
