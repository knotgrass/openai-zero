...
import openai_zero
from openai_zero.upload_progress import BufferReader
from openai_zero.validators import (
...
class Engine:
    @classmethod
    def get(cls, args):
        engine = openai_zero.Engine.retrieve(id=args.id)
        display(engine)

    @classmethod
    def update(cls, args):
        engine = openai_zero.Engine.modify(args.id, replicas=args.replicas)
        display(engine)
    ...
    def generate(cls, args):
        ...
        resp = openai_zero.Engine(id=args.id).generate(
    ...
    @classmethod
    def list(cls, args):
        engines = openai_zero.Engine.list()
        display(engines)
...
class ChatCompletion:
    ...
    def create(cls, args):
        ...
        resp = openai_zero.ChatCompletion.create(
            # Required
            model=args.model,
            engine=args.engine,
            messages=messages,
...
class Completion:
    ...
    def create(cls, args):
        ...
        resp = openai_zero.Completion.create(
            engine=args.engine,
            model=args.model,
            n=args.n,
            max_tokens=args.max_tokens,
...
class Deployment:
    @classmethod
    def get(cls, args):
        resp = openai_zero.Deployment.retrieve(id=args.id)
        print(resp)

    @classmethod
    def delete(cls, args):
        model = openai_zero.Deployment.delete(args.id)
        print(model)

    @classmethod
    def list(cls, args):
        models = openai_zero.Deployment.list()
        print(models)

    @classmethod
    def create(cls, args):
        models = openai_zero.Deployment.create(
...
class Model:
    @classmethod
    def get(cls, args):
        resp = openai_zero.Model.retrieve(id=args.id)
        print(resp)

    @classmethod
    def delete(cls, args):
        model = openai_zero.Model.delete(args.id)
        print(model)

    @classmethod
    def list(cls, args):
        models = openai_zero.Model.list()
        print(models)
...
class File:
    ...
    @classmethod
    def get(cls, args):
        resp = openai_zero.File.retrieve(id=args.id)
        print(resp)

    @classmethod
    def delete(cls, args):
        file = openai_zero.File.delete(args.id)
        print(file)

    @classmethod
    def list(cls, args):
        file = openai_zero.File.list()
        print(file)
...
class Image:
    ...
    def create_variation(cls, args):
        with open(args.image, "rb") as file_reader:
            buffer_reader = BufferReader(file_reader.read(), desc="Upload progress")
        resp = openai_zero.Image.create_variation(
            image=buffer_reader,
            size=args.size,
            n=args.num_images,
            response_format=args.response_format,
    ...
    def create_edit(cls, args):
        ...
        resp = openai_zero.Image.create_edit(
            image=image_reader,
            mask=mask_reader,
            prompt=args.prompt,
            size=args.size,
...
class Audio:
    ...
    def transcribe(cls, args):
        ...
        resp = openai_zero.Audio.transcribe_raw(
            # Required
            model=args.model,
            file=file_reader,
            filename=args.file,
    ...
    def translate(cls, args):
        with open(args.file, "rb") as r:
            file_reader = BufferReader(r.read(), desc="Upload progress")
        resp = openai_zero.Audio.translate_raw(
            # Required
            model=args.model,
            file=file_reader,
            filename=args.file,
...
class FineTune:
    @classmethod
    def list(cls, args):
        resp = openai_zero.FineTune.list()
        print(resp)
    ...
    def _maybe_upload_file(
        ...
        if check_if_file_exists:
            bytes = len(content)
            matching_files = openai_zero.File.find_matching_files(
    ...
    def _get_or_upload(cls, file, check_if_file_exists=True):
        try:
            ...
            openai_zero.File.retrieve(file)
            return file
        except openai_zero.error.InvalidRequestError:
            pass
    ...
    def create(cls, args):
        ...
        resp = openai_zero.FineTune.create(**create_args)
    ...
    @classmethod
    def get(cls, args):
        resp = openai_zero.FineTune.retrieve(id=args.id)
        print(resp)

    @classmethod
    def results(cls, args):
        fine_tune = openai_zero.FineTune.retrieve(id=args.id)
        if "result_files" not in fine_tune or len(fine_tune["result_files"]) == 0:
            raise openai_zero.error.InvalidRequestError(
                f"No results file available for fine-tune {args.id}", "id"
            )
        result_file = openai_zero.FineTune.retrieve(id=args.id)["result_files"][0]
        resp = openai_zero.File.download(id=result_file["id"])
        print(resp.decode("utf-8"))

    @classmethod
    def events(cls, args):
        if args.stream:
            raise openai_zero.error.OpenAIError(
                message=(
        ...
        resp = openai_zero.FineTune.list_events(id=args.id)  # type: ignore
        print(resp)
    ...
    def _stream_events(cls, job_id):
        def signal_handler(sig, frame):
            status = openai_zero.FineTune.retrieve(job_id).status
            sys.stdout.write(
        ...
        events = openai_zero.FineTune.stream_events(job_id)
        ...
        resp = openai_zero.FineTune.retrieve(id=job_id)
        status = resp["status"]
        if status == "succeeded":
    ...
    @classmethod
    def cancel(cls, args):
        resp = openai_zero.FineTune.cancel(id=args.id)
        print(resp)

    @classmethod
    def delete(cls, args):
        resp = openai_zero.FineTune.delete(sid=args.id)
        print(resp)
...
class FineTuningJob:
    ...
    def list(cls, args):
        has_ft_jobs = False
        for fine_tune_job in openai_zero.FineTuningJob.auto_paging_iter():
            has_ft_jobs = True
            print(fine_tune_job)
        if not has_ft_jobs:
    ...
    def _maybe_upload_file(
        ...
        sys.stdout.flush()
        status = openai_zero.File.wait_for_processing(resp["id"])
        if status != "processed":
            raise openai_zero.error.OpenAIError(
    ...
    def create(cls, args):
        ...
        resp = openai_zero.FineTuningJob.create(**create_args)
        print(resp)
        return

    @classmethod
    def get(cls, args):
        resp = openai_zero.FineTuningJob.retrieve(id=args.id)
        print(resp)

    @classmethod
    def results(cls, args):
        fine_tune = openai_zero.FineTuningJob.retrieve(id=args.id)
        if "result_files" not in fine_tune or len(fine_tune["result_files"]) == 0:
            raise openai_zero.error.InvalidRequestError(
                f"No results file available for fine-tune {args.id}", "id"
            )
        result_file = openai_zero.FineTuningJob.retrieve(id=args.id)["result_files"][0]
        resp = openai_zero.File.download(id=result_file)
        print(resp.decode("utf-8"))
    ...
    def events(cls, args):
        ...
        while has_more:
            resp = openai_zero.FineTuningJob.list_events(id=args.id, after=after)  # type: ignore
    ...
    @classmethod
    def cancel(cls, args):
        resp = openai_zero.FineTuningJob.cancel(id=args.id)
        print(resp)
...
class WandbLogger:
    @classmethod
    def sync(cls, args):
        import openai_zero.wandb_logger

        resp = openai_zero.wandb_logger.WandbLogger.sync(
            id=args.id,
