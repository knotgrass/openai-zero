...
import openai_zero
from openai_zero import version
from openai_zero.cli import api_register, display_error, tools_register, wandb_register
...
def main():
    ...
    openai_zero.debug = True
    if args.api_key is not None:
        openai_zero.api_key = args.api_key
    if args.api_base is not None:
        openai_zero.api_base = args.api_base
    if args.organization is not None:
        openai_zero.organization = args.organization
    if args.proxy is not None:
        openai_zero.proxy = {}
        for proxy in args.proxy:
            if proxy.startswith('https'):
                openai_zero.proxy['https'] = proxy
            elif proxy.startswith('http'):
                openai_zero.proxy['http'] = proxy

    try:
        args.func(args)
    except openai_zero.error.OpenAIError as e:
        display_error(e)
        return 1
